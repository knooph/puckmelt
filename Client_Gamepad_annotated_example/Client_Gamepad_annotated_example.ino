...
void setup() {
  ...
  pSecurity->setCapability(ESP_IO_CAP_NONE);                                  //Set security protocol thing? IO capability
  ...
  BLEDevice::setSecurityCallbacks(new BLESecurityCallbacks());               //This seems like a debug tool
  ...
  pBLEScan->setAdvertisedDeviceCallbacks(new MyAdvertisedDeviceCallbacks());  //Then actually sets the settings of the scanner obejct
...
void loop() {
  ...
  if (!connected && doScan) {                                             //Scanning logic
    Serial.println("\nScanning for gamepads...");
    BLEDevice::getScan()->start(5, false);                                //Just specify settings in setup and then just repeatedly start a new scan
    delay(1000);
  }
