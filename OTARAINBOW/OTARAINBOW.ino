...
const char *ssid = "ARDUINOOTA";
const char *password = "CouchPotato";
...
void loop() {
  ...
  for (int &channel : color) {
    channel = (count > 500 ? 0 : count < 250 ? count : 500 - count) * brightness / 250;
    count = (count + 333) % 1000;
  }

  rgbLedWrite(PIN_RGB_LED, color[1], color[2], color[0]);

  if (count == 0) { Serial.println("Boop!"); }
